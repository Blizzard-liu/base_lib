...
public final class CrashUtils {
    ...
    static {
        try {
            PackageInfo pi = BaseApp.Companion.getInstance()
                    .getPackageManager()
                    .getPackageInfo(BaseApp.Companion.getInstance().getPackageName(), 0);
            if (pi != null) {
    ...
    public static void init(final String crashDirPath, final OnCrashListener onCrashListener) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                && BaseApp.Companion.getInstance().getExternalCacheDir() != null)
            defaultDir = BaseApp.Companion.getInstance().getExternalCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        else {
            defaultDir = BaseApp.Companion.getInstance().getCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        }
         Log.d("CrashUtils", "defaultDir   " + defaultDir );
